...
class CooldownManager {
  ...
  static const int cooldownDuration = 30; // Cooldown duration in seconds
  ...
  Future<bool> canColorPixel() async {
    try {
      ...
      if (lastColorTimestamp == null) return true; // No timestamp recorded, can color pixel

      DateTime lastColorTime = DateTime.fromMillisecondsSinceEpoch(lastColorTimestamp);
  ...
  Future<void> recordPixelColored() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      await prefs.setInt(_lastColorTimestampKey, DateTime.now().millisecondsSinceEpoch);
    } catch (e) {
      print('Error recording pixel color timestamp: $e');
    }
  }
  ...
  Future<int> getRemainingCooldownTime() async {
    try {
      ...
      if (lastColorTimestamp == null) return 0; // No timestamp, no cooldown time remaining
      ...
      return remainingTime > 0 ? remainingTime : 0; // Return 0 if cooldown has expired
    } catch (e) {
